...
export default class AddEditGatheringNodeModal extends PureComponent {
  constructor(props) {
    super(props);
    ...
    const { itemName = '', map = '', xCoord = 0, yCoord = 0, note = '' } = this.props.selectedNode;
  ...
  add(e) {
    ...
    Meteor.call('addNewGatheringNode', { ...this.state.form }, err => {
  ...
  edit(e) {
    ...
    Meteor.call('editGatheringNode', { nodeId, ...this.state.form }, err => {
      if (err) console.log(err);
      else this.props.onHide();
    });
  }
  ...
  isSubmitDisabled() {
    ...
    const { form: { itemName, map, xCoord, yCoord } } = this.state;
    return !(itemName && map && xCoord && yCoord);
  }

  render() {
    const { add } = this.props;
    const { form: { itemName, map, xCoord, yCoord, note } } = this.state;
